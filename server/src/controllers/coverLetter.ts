    ...
    try {
        ...
        if (useJobUrl && jobUrl) {
            try {
                ...
                const jobInfoResponse = await getStructuredOutput(
                    prompt,
                    z.object({ jobTitle: z.string(), company: z.string(), jobDescription: z.string() })
                );
        ...
        if (coverLetter._id) {
            const updatedCoverLetter = await CoverLetter.findOneAndUpdate({ _id: coverLetter._id }, coverLetterIn, { new: true });
            return res.status(200).json({ coverLetter: updatedCoverLetter });
        }
