...
<div class="space-y-6" class:animate-pulse={loading}>
    ...
    <div class="flex flex-wrap justify-between gap-2 pt-4 border-t border-gray-200">
        <Button variant="secondary" disabled={loading || formData.id === undefined} on:click={handleDelete}>Delete</Button>

        <div class="flex gap-2">
            <Button variant="ghost" disabled={loading} on:click={handleCancel}>Cancel</Button>
            <Button variant="primary" disabled={loading} on:click={handleSave}>Save</Button>
        </div>
    </div>
</div>
