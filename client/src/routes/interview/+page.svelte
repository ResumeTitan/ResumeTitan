<script lang="ts">
	...
	async function handleGenerateInterview() {
		...
		try {
			...
			if (
				inputMode === 'link' &&
				(err.response?.status === 400 || err.response?.data?.errorType === 'URL_PARSE_ERROR')
			) {
		...
		<div class="form-container">
			...
			<div class="p-4">
				...
				<div class="flex mb-4 gap-0.5">
					<button
						type="button"
						class="flex-1 py-1.5 rounded-t-lg font-bold text-lg transition-colors {inputMode ===
						'manual'
				...
				{:else}
					...
					<div class="mt-4">
						...
						<textarea id="description" class="form-style" rows="10" bind:value={jobDescription}
						></textarea>
					</div>
				{/if}
			</div>
			...
			<div class="form-container mt-8">
				...
				<div class="interview-questions space-y-6">
					{#each interview as question, index}
						<div
							class="question-container p-6 border-2 border-gray-300 rounded-lg bg-white shadow-sm"
						>
							...
							<div class="px-4 mt-2">
								...
								>
									<svg
										class="w-4 h-4 mr-2 transform transition-transform {expandedDetails[index]
											?.example
											? 'rotate-90'
											: ''}"
										fill="currentColor"
	...
	{#if showConfirmPopup}
		<Modal title="Are you sure?" onClose={() => (showConfirmPopup = false)}>
			<p class="text-black mb-4 text-sm">
				Generating new questions will overwrite your current questions and answers.
			</p>
