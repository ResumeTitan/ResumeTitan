<script lang="ts">
	...
	async function handleSaveCoverLetter(exit: boolean) {
		...
		try {
			isLoading = true;
			const response = await api.put<{ coverLetter: CoverLetterType }>(
				`/cover-letter/${coverLetter._id}`,
				coverLetter
			);
			coverLetter = response.coverLetter;
			...
			<div class="flex justify-center w-full">
				<div class="form-container mt-1 max-w-lg w-full">
					...
					<div class="mb-6 p-4 rounded bg-white border border-gray-200">
						...
						{#if inputMode === 'manual'}
							...
							<FormArea
								...
								bind:value={coverLetter.jobDescription}
							/>
						{:else}
							<FormField
								id="jobUrl"
					...
					<div class="p-4 space-y-3">
						...
						{#if coverLetter.letter}
							<Button variant="secondary" fullWidth on:click={handlePrint}>
								Print / Download PDF
							</Button>

							<Button variant="secondary" fullWidth on:click={() => handleSaveCoverLetter(false)}>
								Save Cover Letter
							</Button>

							<Button variant="primary" fullWidth on:click={() => handleSaveCoverLetter(true)}>
								Save and Exit
							</Button>
						{/if}
					</div>
